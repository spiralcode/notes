...
(function( $ ){
  ...
  $.fn.locationPicker = function( options ) {  
    ...
    var $searchInput = $("<input class='lp-search' type='text' placeholder='Type a place name and search'/>");
    $lp.append($searchInput);
    
    var $submit = $("<input class='lp-submit' type='submit' value='Search Place' />");
    $lp.append($submit);
    ...
    var currentLatlng = new google.maps.LatLng(9.498067, 76.338848);
    var myOptions = {
        zoom: 18,
	...
	function findAddress(){
        ...
        if(address == ""){
            $helpText.html('Enter your to location search');
        }else{
...
collectLocation(latLng.lat()+","+latLng.lng());
    ...
    return this.each(function() {        
		...
		$this.focus(function(){		
			...
			$lp.css({
				right: $inputOffset.left-$inputOffset.left+10,
				bottom: $inputOffset.bottom + $this.outerHeight()
			});
			$lp.fadeIn('fast');
